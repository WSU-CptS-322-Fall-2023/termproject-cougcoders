...
def faculty_registration():
    ...
    if rform.validate_on_submit():
        ...
        )  # need to route to faculty_index when we create seperate user index pages
    return render_template(
        "faculty_registration.html", title="Faculty Registration", form=rform
    )
...
def student_registration():
    ...
    if sform.validate_on_submit():
        student = Student(
            ...
            graduation_date=sform.graduation_date.data,
            user_type="Student",
        )
    ...
    return render_template(
        "student_registration.html", title="Student Registration", form=sform
    )
...
def login():
    if current_user.is_authenticated:
        if current_user.user_type == "Student":
            return redirect(
                url_for("routes.index")
            )  # needs to be redirected to student_index
        else:
            return redirect(
                url_for("routes.index")
            )  # needs to be redirected to faculty_index
    lform = LoginForm()
