...
class TestModels(unittest.TestCase):
    ...
    def test_ResearchPosition1(self):
        ...
        db.session.commit()
        self.assertEqual(
            ResearchPosition.query.filter_by(id=1).first().title, "Research Position 1"
        )
    ...
    def test_ResearchPosition2(self):
        ...
        db.session.commit()
        self.assertTrue(
            ProgrammingLanguage.query.first()
            in ResearchPosition.query.filter_by(id=1).first().languages_required
        )
        self.assertTrue(
            Field.query.first()
            in ResearchPosition.query.filter_by(id=1).first().research_fields
        )

    # tests for successful get_faculty method of ResearchPosition class model
    def test_get_faculty(self):
        ...
        self.assertTrue(test_position.get_faculty(), User.query.filter_by().first().id)
        self.assertEqual(len(User.query.all()), 1)
        self.assertEqual(len(ResearchPosition.query.all()), 1)

    # tests for successful get_students method of ResearchPosition class model
    def test_get_students(self):
        ...
        test_app = Application(
            reason="testing app",
            refrence_name="Sakire",
            refrence_email="S.AA@wsu.edu",
            status="Pending",
            student_id=test_student.id,
            research_position_id=test_position.id,
        )
        ...
        self.assertEqual(test_position.get_students(), User.query.all())
        self.assertEqual(len(User.query.all()), 2)
        self.assertTrue(len(ResearchPosition.query.all()), 1)
    ...
    def test_Application1(self):
        ...
        test_app = Application(
            ...
            refrence_email="S.AA@wsu.edu",
            status="Pending",
            student_id=User.query.filter_by().first().id,
        )
        db.session.add(test_app)
        db.session.commit()
    ...
    def test_Application2(self):
        ...
        db.session.add(test_student)
        db.session.commit()
        test_app1 = Application(
            reason="seems interesting",
            refrence_name="Andy Ofallon",
            refrence_email="A.O@wsu.edu",
            status="Pending",
            student_id=User.query.filter_by().first().id,
        )
        ...
        self.assertEqual(Application.query.filter_by(id=1).first().student_id, 1)
        self.assertEqual(
            Application.query.filter_by(id=1).first().student_id,
            Application.query.filter_by(id=2).first().student_id,
        )
